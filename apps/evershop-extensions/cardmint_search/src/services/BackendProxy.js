 ...
 * @param {string} path - API path (e.g., "/api/cm-admin/dashboard/sales-summary")
...
/**
 * Proxy a POST request with JSON body to CardMint backend
...
/**
 * Proxy a POST request with FormData (file upload) to CardMint backend
 * @param {string} path - API path
 * @param {FormData} formData - FormData object with file(s)
 * @returns {Promise<{ok: boolean, status: number, data?: any, error?: string}>}
 */
export async function proxyPostFormData(path, formData) {
  try {
    const url = new URL(path, BACKEND_URL);
    ...
    console.error("[BackendProxy] POST FormData error:", error.message);
    return { ok: false, status: 502, error: error.message, data: null };
  }
}

/**
 * Proxy a DELETE request to CardMint backend
 * @param {string} path - API path
 * @returns {Promise<{ok: boolean, status: number, data?: any, error?: string}>}
 */
export async function proxyDelete(path) {
  try {
    ...
    console.error("[BackendProxy] DELETE error:", error.message);
    return { ok: false, status: 502, error: error.message, data: null };
  }
}
...
export const backendProxy = {
  get: proxyGet,
  post: proxyPost,
  postFormData: proxyPostFormData,
  delete: proxyDelete,
};
