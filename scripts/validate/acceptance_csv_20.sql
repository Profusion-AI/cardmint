-- CardMint Acceptance Gate
-- Validates extraction, retrieval, and schema health against ground truth
--
-- Usage: sqlite3 apps/backend/data/cardmint.db < scripts/validate/acceptance.sql
--
-- Expected ground truth: 20card-22oct.csv in workspace root

.mode column
.headers on

-- Ground Truth Import
-- Load the 20-card reference dataset from PriceCharting
...
.mode csv
.import 20card-22oct.csv ground_truth_raw
...
SELECT CASE
  WHEN (SELECT COUNT(*) FROM ground_truth) = 0 THEN 'NOTE: No ground truth CSV found (20card-22oct.csv) — accuracy gates will show N/A/RED for dev DBs.'
  ELSE 'Ground truth present — accuracy gates active.'
END AS "Info";

-- Get most recent session for matching scans
-- Note: For Run #1 compatibility, we select the most recent 20 scans in chronological order
-- Run #2+ will use proper operator_sessions filtering
...
-- Summary based on recent_session_scans (last 20) as session proxy
SELECT
  'accepted=' || (
...
SELECT
  CASE
    ...
    LEFT JOIN products p ON i.product_uid = p.product_uid
        WHERE s.status = 'ACCEPTED' AND p.manual_reason_code IS NOT NULL
      ) / NULLIF((SELECT COUNT(*) FROM recent_session_scans WHERE status = 'ACCEPTED'), 0)) > 25.0
      OR (
        SELECT COUNT(*)
        FROM recent_session_scans s
        LEFT JOIN items i ON s.item_uid = i.item_uid
