#!/usr/bin/env python3
"""Baseline Pokémon Card Identification against LM Studio vision model.

This script prepares a deterministic test batch for the first five cards, sends
encoded images to the LM Studio OpenAI-compatible endpoint, and records model
predictions alongside ground truth metadata.

It follows LM Studio documentation on:
- OpenAI-compatible endpoints: https://lmstudio.ai/docs/app/api/endpoints/openai
- Structured output enforcement: https://lmstudio.ai/docs/app/api/structured-output
- Tooling integration (future use): https://lmstudio.ai/docs/app/api/tools
"""
...
def identify_card(
    client: OpenAI, image_path: Path, *, max_tokens: int = 256
...
def determine_output_path(prefix: str = "id-results") -> Path:
...
def main(args: argparse.Namespace) -> int:
    ...
    print(f"Starting CardMint baseline test on {len(image_paths)} cards...")
    ...
    if not wait_for_model_ready(client):
        print("❌ Model not ready - aborting baseline test", file=sys.stderr)
        return 1

    # Model is ready - proceed with batch evaluation
    print(f"🚀 Model ready - starting batch evaluation...\n")
