#!/usr/bin/env python3
"""CardMint Baseline V2 - Phase 4D Precision Pass with Kyle's 777/42 Parameters.

This is the refreshed baseline test incorporating Kyle's validated 777/42 configuration
that resolves JSON truncation issues and achieves superior variance control.

BASELINE V2 CONFIGURATION (Phase 4D):
...
5. Performance targets: ~18s average, ≥95% accuracy, <10% variance
...
- 512/30 config: 100% JSON truncation failure rate
- 777/42 config: 17.6s avg, 5.7% variance, 0% JSON failures
- Kyle's hypothesis confirmed: Higher token limits essential for stability
- Phase 4D: Precision pass removing sampling overhead for deterministic output

This replaces the failed Phase 4C 512/30 config as the production standard.
Based on token parameter regression testing - 25 Sep 2025.
"""
...
def main():
    ...
    parser.add_argument("--cards-limit", type=int, help="Limit number of cards for testing (deprecated, use --size)")
    parser.add_argument("--size", type=int, help="Number of cards to test (5, 10, 15, 50, 200, etc.)")
    parser.add_argument("--test-name", type=str, default="Baseline-V2", help="Test name for results")
    parser.add_argument("--mini", action="store_true", help="Run mini test (5 cards, equivalent to --size=5)")
    parser.add_argument("--medium", action="store_true", help="Run medium test (15 cards, equivalent to --size=15)")
    ...
    # Determine card limit (priority: --size > --mini/--medium > --cards-limit)
    card_limit = None
    if args.size:
        card_limit = args.size
        # Adjust test name based on common sizes
        if card_limit == 5:
            args.test_name = f"{args.test_name}-Mini"
        elif card_limit == 15:
            args.test_name = f"{args.test_name}-Medium"
        elif card_limit == 50:
            args.test_name = f"{args.test_name}-50Card"
        elif card_limit == 200:
            args.test_name = f"{args.test_name}-200Card"
        else:
            args.test_name = f"{args.test_name}-{card_limit}Card"
    elif args.mini:
        card_limit = 5
        args.test_name = f"{args.test_name}-Mini"
    elif args.medium:
