#!/usr/bin/env python3
"""Phase 2.5 Model Swap Test - google/gemma-3-12b Performance Comparison.

This script implements a controlled experiment by maintaining all Phase 2 optimizations
while swapping the model from mistralai/magistral-small-2509 to google/gemma-3-12b.

Phase 2.5 maintains all Phase 2 parameters:
- Context reduction: 1K tokens (same as Phase 2)
- Max tokens: 50 tokens (same as Phase 2)
- Enhanced reasoning suppression with explicit JSON-only instruction (same as Phase 2)
- Same test set and accuracy validation

Target: Isolate model performance impact by changing only the model while keeping
all other optimization parameters constant.
"""
...
DEFAULT_MODEL_ID = "google/gemma-3-12b"  # PHASE 2.5: Model swap from magistral-small-2509

# PHASE 2.5: Same aggressive reasoning suppression as Phase 2
SYSTEM_PROMPT = (
...
def identify_card(
    client: OpenAI, image_path: Path, *, max_tokens: int = 50  # PHASE 2.5: Same 50 token limit as Phase 2
    ...
    response = client.chat.completions.create(
        ...
        # PHASE 2.5: Same 1K context reduction as Phase 2
...
def determine_output_path(prefix: str = "id-results-phase2_5") -> Path:
...
def wait_for_model_ready(client: OpenAI, max_wait_seconds: int = 300) -> bool:
    ...
    Google Gemma-3-12B model loading may take 3-4 minutes for cold start.
...
def main(args: argparse.Namespace) -> int:
    ...
    print(f"🔬 Starting CardMint PHASE 2.5 MODEL SWAP test on {len(image_paths)} cards...")
    print(f"Target: {DEFAULT_SERVER_URL} | Model: {DEFAULT_MODEL_ID}")
    print("Phase 2.5 Configuration: Same as Phase 2 (1K context, 50 max_tokens) with model swap")
    print("=" * 80)

    # Wait for model to be ready (critical for cold-start)
    client = OpenAI(base_url=DEFAULT_SERVER_URL, api_key="lm-studio")
    if not wait_for_model_ready(client):
        print("❌ Model not ready - aborting Phase 2.5 test", file=sys.stderr)
        return 1

    # Model is ready - proceed with batch evaluation
    print(f"🔬 Model ready - starting PHASE 2.5 batch evaluation...\n")
    results, tally, timing_stats = evaluate_batch(image_paths)
    output_path = determine_output_path()
    write_results(results, output_path)

    # Performance Summary
    print(f"\n📊 PHASE 2.5 PERFORMANCE METRICS")
    ...
    print(f"\n💾 Phase 2.5 results saved to: {output_path}")
    return 0
