#!/usr/bin/env python3
"""Phase 1 Optimized Pokémon Card Identification - A/B Test Implementation.

This script implements Phase 1 "Quick Win" optimizations from the inference speed plan:
1. Context window reduction: 8K → 2K tokens
2. Max tokens reduction: 256 → 75 tokens
3. Reasoning suppression: Enhanced prompt to discourage reasoning traces

Target: Maintain ≥95% accuracy while reducing inference time by 20-30%.
Based on pcis_baseline.py with performance optimizations applied.
"""
...
# OPTIMIZATION 3: Enhanced system prompt with reasoning suppression
SYSTEM_PROMPT = (
    ...
    "Identify the Pokemon card directly without explaining your reasoning process. "
    "Respond immediately with only the JSON containing name, hp, and set_number."
)
...
def identify_card(
    client: OpenAI, image_path: Path, *, max_tokens: int = 75  # OPTIMIZATION 2: Reduced from 256
    ...
    response = client.chat.completions.create(
        ...
        # OPTIMIZATION 1: Context window reduction from default (likely 8K) to 2K
        extra_body={"context_length": 2048}
    )
...
def determine_output_path(prefix: str = "id-results-optimized") -> Path:
...
def main(args: argparse.Namespace) -> int:
    ...
    print(f"🚀 Starting CardMint OPTIMIZED test on {len(image_paths)} cards...")
    print(f"Target: {DEFAULT_SERVER_URL} | Model: {DEFAULT_MODEL_ID}")
    print("Phase 1 Optimizations: 2K context, 75 max_tokens, reasoning suppression")
    print("=" * 70)

    # Wait for model to be ready (critical for cold-start)
    client = OpenAI(base_url=DEFAULT_SERVER_URL, api_key="lm-studio")
    if not wait_for_model_ready(client):
        print("❌ Model not ready - aborting optimized test", file=sys.stderr)
        return 1

    # Model is ready - proceed with batch evaluation
    print(f"🚀 Model ready - starting OPTIMIZED batch evaluation...\n")
