/**
 * Export reconciliation CSV for scans accepted/locked without a canonical match.
 * Reads from cardmint_dev.db (override via SQLITE_DB env) and writes to exports/reconciliation-YYYYMMDD-HHMM.csv.
 *
 * Fields: scan_id, status, canonical_locked, reconciliation_status, card_name, collector_no, set_name, hp_value, condition_bucket, cm_card_id, product_sku, listing_sku, capture_uid, updated_at
 */
...
const dbPath = process.env.SQLITE_DB || join(process.cwd(), "apps/backend/cardmint_dev.db");
