...
|----------|---------|----------|
| `CARDMINT_ADMIN_API_KEY` | Bearer token for `/api/admin/*` endpoints | Yes for admin endpoints |
| `CAPTURE_INTERNAL_KEY` | Secret header for calibration/capture endpoints | No (backward compat) |
| `DISPLAY_TOKEN` | Token for `/api/stock-summary/*` endpoints | No (backward compat) |
